...
定数・スタイル・キャンパス定義は `campuses.ts` に集約 (`CAMPUSES`)。地図本体は `MapView.svelte`、選択時の詳細表示は `DetailPanel.svelte`。
...
- 名前付き建物のみクリック可 (クリーム色)。名前なしは情報が無いのでグレーアウト + クリック無反応
- 建物クリック → `properties.id` でフィルタしてハイライト + `DetailPanel` 表示 (建物情報と階層図を 1 枚に統合。中間カードは無し)
...
- **工学部2号館 (`relation/9693129`)** はフロア図画像付き (`static/data/floors/eng2/*.png`)。JSME 2022 年次大会の会場案内から拝借した**暫定**で、本来の構内データに差し替える前提
- 駒場キャンパスの建物群は主にメタ情報 (`websiteUrl` 等) のみ。フロア図が無くても `DetailPanel` は建物情報 + 関連サイトリンクを出す
- 部屋に建物内座標はない → 地図上での部屋ハイライトは不可。`DetailPanel` がフロア図画像 + 部屋リストを出すだけ
- 部屋検索は `searchRooms()` (号室番号/部屋名/会場記号/建物名にマッチ)。`MapView` の検索バーが建物検索と統合し、部屋ヒット→親建物へ flyTo + 該当階・部屋を `DetailPanel` で開く
- 建物に階層図があるかは `hasFloors(id)`。検索バーは `hasFloors` な建物を空欄フォーカス時の「クイックアクセス」候補に出す
